...
def test_armada_create_armada_pod_spec():
    pod_spec = create_armada_pod_spec(
        "fake-step-name",
        "fake-run-id",
        metaflow.ArmadaDecorator.defaults,
        ["sleep 10"],
        {},
        {},
    )
    ...
    assert (
        pod_spec[0].pod_spec.containers[0].name == "metaflow-fake-step-name-fake-run-id"
    )
...
@patch("metaflow.armada._get_client")
